                    ...
                    if (_ != 'name') {
                        ...
                        $(`#page_shop .scroll-container .collapsible #${index} .collapsible-body #${_}`).hover(function() {                       
                            $( this ).click(function() {                        
                                ...
                                $.post('https://rsg_stable/loadHorse', JSON.stringify({ horseModel: $(this).attr('id') }));
                            });                       
        ...
        for (const [ind, tab] of Object.entries(event.data.myHorsesData)) {
            ...
            $(`#page_myhorses .scroll-container .collapsible #${HorseID}`).hover(function() {  
                $( this ).click(function() {                    
                    ...
                    $.post('https://rsg_stable/loadMyHorse', JSON.stringify({ IdHorse: HorseID, horseModel: HorseIdModel, HorseComp: componentsh}));
                });                         
            }, function() {});
        }
    }
...
function confirm(){
    $.post('https://rsg_stable/CloseStable')
...
$(".button-right").on('click', function() {
  ...
  //  var component = text.split(' ')[0];
    ...
    $.post('https://rsg_stable/'+component, JSON.stringify({ id: nValue }));
});
...
function buyHorse(Modelhor, price, isGold) {        
    ...
    if (isGold) {        
        $.post('https://rsg_stable/BuyHorse', JSON.stringify({ ModelH: Modelhor, Gold: price, IsGold: isGold }));
    } else {
        $.post('https://rsg_stable/BuyHorse', JSON.stringify({ ModelH: Modelhor, Dollar: price, IsGold: isGold }));    
    }    
}


function SelectHorse(IdHorse) {    
    $.post('https://rsg_stable/selectHorse', JSON.stringify({ horseID: IdHorse }))    
}


function SellHorse(IdHorse) {    
    $.post('https://rsg_stable/sellHorse', JSON.stringify({ horseID: IdHorse }))
