...
local function OpenStable()
    ...
    TriggerServerEvent("rsg_stable:AskForMyHorses")
end
...
local function SetHorseName(data)
	...
	CreateThread(function()
		...
		if (GetOnscreenKeyboardResult()) then
            ...
            TriggerServerEvent('rsg_stable:BuyHorse', data, HorseName)
...
local function CloseStable()
	...
	if DadosEncoded ~= "{}" then
		TriggerServerEvent("rsg_stable:UpdateHorseComponents", dados, IdMyHorse, MyHorse_entity )
	end
end
...
local function InitiateHorse(atCoords)
    ...
    if horseModel == nil and horseName == nil then
        TriggerServerEvent("rsg_stable:RequestMyHorseInfo")
...
local function WhistleHorse()
    if SpawnplayerHorse ~= 0 then
        ...
        TriggerServerEvent('rsg_stable:CheckSelectedHorse')
        Wait(100)
        InitiateHorse()
    end
end
...
RegisterNUICallback("selectHorse", function(data)
	TriggerServerEvent("rsg_stable:SelectHorseWithId", tonumber(data.horseID))
end)

RegisterNUICallback("sellHorse", function(data)
	DeleteEntity(showroomHorse_entity)
	TriggerServerEvent("rsg_stable:SellHorseWithId", tonumber(data.horseID))
	TriggerServerEvent("rsg_stable:AskForMyHorses")
...
RegisterNetEvent("rsg_stable:SetHorseInfo", SetHorseInfo)

RegisterNetEvent('rsg_stable:client:UpdadeHorseComponents', function(horseEntity, components)
    for _, value in pairs(components) do
        NativeSetPedComponentEnabled(horseEntity, value)
    end
end)

RegisterNetEvent("rsg_stable:ReceiveHorsesData", function(dataHorses)
	SendNUIMessage(
		{
...
CreateThread(function()
    while true do
		...
		if Citizen.InvokeNative(0x91AEF906BCA88877, 0, 0x4CC0E2FE) then -- [B]                     
			if SpawnplayerHorse ~= 0 then
...
function InvHorse()
    if SpawnplayerHorse ~= 0 then
		...
		if #(pcoords - hcoords) <= 1.7 then
			TriggerEvent('rsg_stable:client:horseinventory')
		else
...
-- horse inventory
RegisterNetEvent('rsg_stable:client:horseinventory', function()
...
-- feed horse
RegisterNetEvent('rsg_stable:client:feedhorse')
AddEventHandler('rsg_stable:client:feedhorse', function(itemName, increase)
...
-- brush horse
RegisterNetEvent('rsg_stable:client:brushhorse')
AddEventHandler('rsg_stable:client:brushhorse', function(itemName)
